...
var createBook = function(contentRoot, content, children) {
  return Q.nfcall(temp.mkdir, 'gitbook-tester')
    .then(function(dirPath) {
      var bookFS = {root: dirPath, contentRoot: path.join(dirPath, contentRoot)};
      return ensureDirectory(bookFS.contentRoot)
        .then(function() {
          return bookFS;
        });
    })
    .then(function(bookFS) {
      ...
      var pagesPromises = children.map(function (page) {
        ...
        var nestedPageDirectory = path.join(bookFS.contentRoot, pageSubDirectory);

        return ensureDirectory(nestedPageDirectory)
          .then(
            function () {
      ...
      var summary = Q.nfcall(fs.writeFile, path.join(bookFS.contentRoot, 'SUMMARY.md'), summaryContent);
      var readme = Q.nfcall(fs.writeFile, path.join(bookFS.contentRoot, 'README.md'), content);
...
var installBookJson = function(bookFS, bookJson, modules, withLocalPlugins) {
  ...
  return Q.nfcall(fs.writeFile, path.join(bookFS.root, 'book.json'), JSON.stringify(book))
    .then(function(){return bookFS;});
};

var runCommand = function(logger, command, args, opts) {
...
var attachLocalPlugins = function(bookFS, localModules) {
    var nodeModulesPath = path.join(bookFS.root, 'node_modules');
    return Q.nfcall(fs.mkdir, nodeModulesPath) // create node_modules directory
      ...
      .then(function(){return bookFS;}); // return book path
};
...
var install = function(logger, bookFS) {
  return runCommand(logger, gitbookRunnablePath(), ['install', bookFS.root])
    .then(function(){return bookFS;});
};

// execute 'gitbook build /temp/path/to/generated/book'
var includeFiles = function(bookFS, files) {
  var promises = files.map(function(file){
    var fullFilePath = path.join(bookFS.contentRoot, file.path);
...
var runBeforeBuild = function(logger, bookFS, commands) {
  var promise = Q.resolve();
  return commands.reduce(function(acc, commandAndArgs) {
    return acc.then(function() {
      return runCommand(logger, commandAndArgs[0], commandAndArgs[1], {cwd: bookFS.root});
    });
  }, promise);
};

// execute 'gitbook build /temp/path/to/generated/book'
var build = function(logger, bookFS) {
  var command = ['build', bookFS.root];
  ...
  return runCommand(logger, gitbookRunnablePath(), command)
    .then(function(){return bookFS;});
};

// traverse rendered book and return all html files found
var _readFiles = function(bookFS, extension) {
  var deferred = Q.defer();
  var find = finder(path.join(bookFS.root, '_book'));
  var pages = [];
...
var processFiles = function(bookFS, files) {

  var promises = _.flatten(files).map(function(filename){
    return Q.nfcall(fs.readFile, filename, 'utf-8')
      .then(function(fileContent) {
        var result = {
          path : path.relative(path.join(bookFS.root, '_book'), filename)
        };
  ...
  return Q.all(promises).then(function(results){
    ...
    resultsObj.root = bookFS.root;
    resultsObj.contentRoot = bookFS.contentRoot;
...
var execute = function(htmlContent, bookJson, localModules, files, pages, beforeBuild, logger) {
  winston.level = process.env.DEBUG ? 'debug' : 'warn';

  var that = this;
  var contentRoot = (bookJson && bookJson.root) || '.';

  var modules = preprocessLocalModules(localModules);

  return createBook(contentRoot, htmlContent, pages)
    .then(function(bookFS) {
      return attachLocalPlugins(bookFS, modules)
        .then(installBookJson.bind(that, bookFS, bookJson, modules, false))
        .then(install.bind(that, logger, bookFS))
        .then(installBookJson.bind(that,  bookFS, bookJson, modules, true))
        .then(includeFiles.bind(that, bookFS, files))
        .then(runBeforeBuild.bind(that, logger, bookFS, beforeBuild))
        .then(build.bind(that, logger, bookFS))
        .then(readFiles.bind(that, bookFS))
        .then(processFiles.bind(that, bookFS));
    });
};
...
// run an external command after plugins and book.json
// have been set up and before running gitbook build.
Builder.prototype.beforeBuild = function(command, args) {
  this._beforeBuild.push([command, args]);
  return this;
};

// specify a winston logger for capturing or redirecting output from
// gitbook and other external commands.
// defaults to the default winston logger.
Builder.prototype.withLogger = function(logger) {
  this._logger = logger;
  return this;
};
...
Builder.prototype.create = function() {
  return execute(this._content, this._bookJson, this._modules, this._files, this._pages, this._beforeBuild, this._logger);
};
