...
var install = function(bookFS) {
  return runCommand(gitbookRunnablePath(), ['install', bookFS.root])
    .then(function(){return bookFS;});
};
...
var runBeforeBuild = function(bookFS, commands) {
  var promise = Q.resolve();
  return commands.reduce(function(acc, commandAndArgs) {
    return acc.then(function() {
      return runCommand(commandAndArgs[0], commandAndArgs[1], {cwd: bookFS.root});
    });
  }, promise);
};

// execute 'gitbook build /temp/path/to/generated/book'
var build = function(bookFS) {
...
var execute = function(htmlContent, bookJson, localModules, files, pages, beforeBuild) {
  ...
  return createBook(contentRoot, htmlContent, pages)
    .then(function(bookFS) {
      return attachLocalPlugins(bookFS, modules)
        ...
        .then(install.bind(that, bookFS))
        .then(installBookJson.bind(that, bookFS, bookJson, modules, true))
        .then(includeFiles.bind(that, bookFS, files))
        .then(runBeforeBuild.bind(that, bookFS, beforeBuild))
        .then(build.bind(that, bookFS))
...
Builder.prototype.create = function() {
  return execute(this._content, this._bookJson, this._modules, this._files, this._pages, this._beforeBuild);
};
